The complete AI multi-agent blueprint for shipping “Weather Whether” 
...
**Core design contract (Weather Whether).** The PM and every agent must align implementation, tasks, and reviews with **`docs/GAME_DESIGN.md` v2** (authoritative GDD). The file `docs/Building Whether_ A Weather-Powered Puzzle Game from Zero to Launch.md` remains useful for toolkit / pipeline / study references but is **not** the rules source for mechanics. Do not add orthogonal mechanics without an explicit GDD change and producer sign-off.
