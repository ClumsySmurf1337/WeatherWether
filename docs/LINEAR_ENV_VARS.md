...
| `WEATHER_COPILOT_NO_ALLOW_ALL` | If **`1`** / **`true`**, lane scripts do **not** pass **`--allow-all`** (default **on** — same as **`--allow-all-tools --allow-all-paths --allow-all-urls`**, per **`copilot --help`**). Optional: set **`COPILOT_ALLOW_ALL`** env per CLI docs |
