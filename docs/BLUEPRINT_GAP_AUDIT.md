...
| **CI** | `.github/workflows/ci.yml` | **Node/Linear tooling only**; prints placeholder — **does not run Godot headless** yet. |
...
- CI does not execute Godot validation (local `validate.ps1` is source of truth until CI image exists).
...
1. **CI:** Godot headless on runner + `validate.ps1` (GUT + level validation) — mirror local checks.
