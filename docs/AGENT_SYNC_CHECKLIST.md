...
- [ ] Task entry points still live in `tools/tasks/*.ps1` (including `prepare-editor-lane-worktrees.ps1`, `lane-ship.ps1` + `lane-ship-lib.ps1` (unpushed detection), `lane-ship-all.ps1` / `npm run lane:ship:lanes`, `run-lane-terminal.ps1` auto-ship + `.weather-lane-issue.txt`, `qa-lane-pr-batch.ps1` / `npm run qa:agent` (pre-flight ship lanes 1–3, then merge PRs), `lane-worktrees-reset-for-next-cycle.ps1`).
