...
- [ ] Task entry points still live in `tools/tasks/*.ps1` (including `prepare-editor-lane-worktrees.ps1`, `lane-ship.ps1` + `lane-ship-lib.ps1` (unpushed detection; `linear:verify-issue-for-ship` before PR), `lane-ship-all.ps1` / `npm run lane:ship:lanes`, `run-lane-terminal.ps1` auto-ship + `.weather-lane-issue.txt`, `qa-lane-pr-batch.ps1` / `npm run qa:agent` (pre-flight ship lanes 1–3, then merge PRs), `git-sync-main.ps1` (fetch + ff `main` before each handoff and after each merge), `linear-complete-from-merged-prs.ps1` / `npm run linear:complete-merged-lane-prs`, `lane-worktrees-reset-for-next-cycle.ps1`). **Cursor:** `.vscode/tasks.json` (daily + parallel lanes + simple-flow compound + QA) matches **`docs/DAILY.md`**.
