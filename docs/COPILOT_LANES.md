...
| **`WEATHER_COPILOT_NO_ALLOW_ALL`** | Set to **`1`** to **disable** default **`--allow-all`** (otherwise non-interactive runs often fail with “Permission denied and could not request permission from user”) |
...
Lane scripts invoke **`copilot`** with **`--no-ask-user`** plus **`--allow-all`** (tools, paths, and URLs — matches **`copilot -p "…" --allow-all`** in **`copilot --help`**).
