...
**PM board + doc-grounded drafts (not part of simple flow):** **`npm run daily:full:apply:lanes`** / **`npm run workflow:simple`** regenerate **`linear:pm-assignments`** during lane prep but **do not** run **`linear:pm-organize`** or **`linear:pm-doc-intake`**. When you want phase/priority alignment and fresh GDD/blueprint issue drafts before lanes work, run **`npm run linear:pm-handoff`** (dry-run safe on Linear except assignment regen) or **`npm run linear:pm-handoff:apply`** to push board changes. See **`docs/PM_AGENT_LINEAR.md`**.
