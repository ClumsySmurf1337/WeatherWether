...
- **Several lane PRs at once** (heads `agent/cursor-lane-1` …): from repo root **`npm run qa:agent`** (alias: **`npm run qa:lane-prs`**) — `tools/tasks/qa-lane-pr-batch.ps1` processes each open lane PR in order with **`SyncMainBeforeValidate`** (conflicts → **`qa:repair-merge`** flow), merges, moves Linear to **Done**, **`npm run worktrees:sync`**, appends **`docs/CHANGELOG_LANES.md`**, then **resets lane branches** from **main** (use **`-SkipResetLaneBranches`** to skip reset, or run **`npm run lane:next-cycle`** manually later). **`npm run qa:lane-prs:quick`** passes **`-SkipChecksWatch`** if CI is already green.
