...
- **Cursor Tasks** (`.vscode/tasks.json`) — **Weather Whether — Daily apply:lanes, then parallel lane agents** runs **`daily:full:apply:lanes`** then **three lane agents in parallel**; each lane terminal **closes when that lane finishes** (`presentation.close`). **Weather Whether — QA agent (lane PRs)** runs **`npm run qa:agent`**. **Weather Whether — Simple flow: daily+lanes, then QA (steps 1–2)** chains those two in order. See **`docs/DAILY.md`** for the full loop (step **3** = repeat step **1**, and step **2** when merging).
