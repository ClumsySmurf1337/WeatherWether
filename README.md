...
- **Cursor Tasks** (`.vscode/tasks.json`) — **Weather Whether — Daily apply:lanes, then parallel lane agents** runs **`daily:full:apply:lanes`** then **three `cursor-agent` lanes** in parallel; lane terminals **stay open** when each lane finishes (`presentation.close: false` — inspect Copilot / agent output and exit codes). **Parallel Copilot CLI lanes:** **Daily apply:lanes, then parallel Copilot lanes** + **Lane N Copilot** run **`copilot -p … --no-ask-user`** per worktree (fallback: **`-SkipCopilotRun`** for prompt file only). See **`docs/COPILOT_LANES.md`**. **QA agent** unchanged. **`npm run workflow:simple`** vs **`npm run workflow:simple:copilot`**. See **`docs/DAILY.md`**.
...
- `npm run lane:ship -- -LaneIndex <1-3>` — commit + push + open PR when a lane worktree has **uncommitted changes** or **commits not yet on `origin/main`** (Linear id from **`.weather-lane-issue.txt`** after `resume-pickup`, or **`-LinearId WEA-###`**); runs **`validate.ps1 -GodotProjectPath`** (GUT + levels when GUT is installed), then **`linear:verify-issue-for-ship`**. **`-SkipValidate`** / **`-SkipLinearVerify`** skip those gates.
- `npm run lane:ship:lanes` — run **`lane:ship`** for lanes **1–3** (recovery when agents left local commits without a PR)
- `npm run qa:agent` (alias: **`npm run qa:lane-prs`**) — **Pre-flight:** ship lanes **1–3** if a worktree is **dirty** or **ahead of `origin/main`** (opens PRs when there is a real diff vs **main**). Then QA **open** PRs with head **`agent/cursor-lane-*`**: merge main, validate, merge, **Linear Done**, append **`docs/CHANGELOG_LANES.md`** (then **commit + push** that file on **main** when new lines were added), `worktrees:sync`, reset lane branches. **`-SkipChangelogPush`** keeps the log edit local. **`-SkipPreflightShip`** skips the ship scan; **`-SkipResetLaneBranches`** skips branch reset; **`npm run lane:next-cycle`** if you skipped reset
