...
- **Cursor Tasks** (`.vscode/tasks.json`) — **Weather Whether — Daily apply:lanes, then parallel lane agents** runs **`daily:full:apply:lanes`** then **three `cursor-agent` lanes** in parallel; each lane terminal **closes when that lane finishes** (`presentation.close`). **Parallel Copilot lanes:** **Daily apply:lanes, then parallel Copilot lanes** + **Lane N Copilot** tasks write **`WEATHER_COPILOT_LANE_PROMPT.md`** and print **GitHub Copilot Chat (Agent)** steps — see **`docs/COPILOT_LANES.md`**. **Weather Whether — QA agent (lane PRs)** runs **`npm run qa:agent`**. **Simple flow** (Cursor) vs **Simple flow Copilot** (compound Tasks) vs **`npm run workflow:simple`** / **`npm run workflow:simple:copilot`**. See **`docs/DAILY.md`**.
