...
**Lane:** 1 | **Role:** gameplay-programmer | **Worktree:** this folder

Read **`AGENTS.md`**, **`.github/copilot-instructions.md`**, and **`.claude/CLAUDE.md`** before coding (same contract as Cursor; role file: **`.claude/agents/gameplay-programmer.md`**).

---
**Copilot lane launcher (Weather Whether)**

- `linear:resume-pickup` for role **gameplay-programmer** was **already run** from the main repo. **`.weather-lane-issue.txt`** in this worktree holds the **WEA-###** marker for shipping.
...
- After implementation: run **`pwsh <MAIN_REPO>/tools/tasks/validate.ps1 -GodotProjectPath (Get-Location).Path`** until green, then from **main**: **`npm run lane:ship -- -LaneIndex 1`** or let **`npm run qa:agent`** preflight ship.
...
1. Run: `npm run linear:resume-pickup -- --role=gameplay-programmer --apply` (from this folder). This resumes your in-progress lane work first; if none exists, it claims a Todo issue. If nothing is available, say so and stop.
...
4. Open **`.claude/agents/gameplay-programmer.md`** — same role contract as Cursor/Claude; follow it for this lane.
