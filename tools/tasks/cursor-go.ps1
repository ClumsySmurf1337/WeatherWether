...
if ($EditorLaneTerminals) {
    ...
    Write-Host ""
    Write-Host "    Each task runs: resume-pickup (main repo .env.local) -> cd worktree -> ``cursor chat`` (lane prompt)."
    Write-Host "    No manual typing in the terminal. If 3 parallel chats fight for one Cursor UI, run lanes one at a time from Tasks.`n"
} elseif (-not $SkipSessionLaunch) {
    Write-Host "`n[5/5] Launch parallel Cursor session (external PowerShell + cursor chat)"
